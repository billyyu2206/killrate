 ...
 */
@Component
public class T1S60ClearListener extends BaseClearListener implements ClearKillRate{
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	@EventListener(condition="#event.clearKillRateVo.lottery == 't1s60'")
	...
	@Bean
	public Queue t1S60ClearQueue() {
        return new AnonymousQueue();
    }
	
	@Bean
    public Binding bindingT1S60(@Qualifier(ClearRateMqConfig.CLEAR_RATE_EXCHANGE) FanoutExchange fanoutExchange, Queue t1S60ClearQueue) {
        return BindingBuilder.bind(t1S60ClearQueue).to(fanoutExchange);
    }
	...
	public void clearResult(ClearKillRateVo vo ,Message message, Channel channel) throws IOException {
		try {
			...
			logger.error("t1S60ClearQueue receive error ", e);
		}finally {
